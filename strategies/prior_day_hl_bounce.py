...
class PriorDayHlBounce:
    ...
    def _update_session(self, tick: Tick) -> None:
        ...
        if self._current_high is not None and tick.price > self._current_high:
            self._current_high = tick.price
        if self._current_low is not None and tick.price < self._current_low:
            self._current_low = tick.price
    ...
    def check(self, tick: Tick, **kwargs: Any) -> Signal | None:
        ...
        if not self.use_confirmation:
            level_price = (
                self._prior_high if self._active_level == "HIGH" else self._prior_low
            )
